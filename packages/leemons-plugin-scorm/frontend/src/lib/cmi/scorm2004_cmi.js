...
export class CMIInteractionsObject extends BaseCMI {
  ...
  set type(type) {
    ...
    } else if (check2004ValidFormat(type, scorm2004_regex.CMIType)) {
      this.#type = type;
    }
  }
  ...
  set timestamp(timestamp) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(timestamp, scorm2004_regex.CMITime)) {
  ...
  set weighting(weighting) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(weighting, scorm2004_regex.CMIDecimal)) {
          ...
          for (let i = 0; i < nodes.length; i++) {
            if (response_type?.delimiter2) {
              ...
              if (values.length === 2) {
                ...
                } else if (!values[1].match(new RegExp(response_type.format2))) {
                  throwTypeMismatchError();
                }
              } else {
                throwTypeMismatchError();
              }
            } else if (!nodes[i].match(formatRegex)) {
              throwTypeMismatchError();
  ...
  set latency(latency) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(latency, scorm2004_regex.CMITimespan)) {
  ...
  set description(description) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(description, scorm2004_regex.CMILangString250, true)) {
...
export class CMIObjectivesObject extends BaseCMI {
  ...
  set success_status(success_status) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(success_status, scorm2004_regex.CMISStatus)) {
  ...
  set completion_status(completion_status) {
    if (this.initialized && this.#id === '') {
      throwDependencyNotEstablishedError();
    } else if (check2004ValidFormat(completion_status, scorm2004_regex.CMICStatus)) {
...
export class CMICommentsObject extends BaseCMI {
  ...
  set comment(comment) {
    if (this.initialized && this.#readOnlyAfterInit) {
      throwReadOnlyError();
    } else if (check2004ValidFormat(comment, scorm2004_regex.CMILangString4000, true)) {
  ...
  set location(location) {
    if (this.initialized && this.#readOnlyAfterInit) {
      throwReadOnlyError();
    } else if (check2004ValidFormat(location, scorm2004_regex.CMIString250)) {
...
export class CMI extends BaseCMI {
  ...
  set completion_threshold(completion_threshold) {
    !this.initialized ? (this.#completion_threshold = completion_threshold) : throwReadOnlyError();
  }
  ...
  set learner_name(learner_name) {
    !this.initialized ? (this.#learner_name = learner_name) : throwReadOnlyError();
  }
  ...
  set scaled_passing_score(scaled_passing_score) {
    !this.initialized ? (this.#scaled_passing_score = scaled_passing_score) : throwReadOnlyError();
  }
  ...
  getCurrentTotalTime() {
    ...
    return Util.addTwoDurations(this.#total_time, sessionTime, scorm2004_regex.CMITimespan);
  }
